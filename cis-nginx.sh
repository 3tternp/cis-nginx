...
risk="Medium"
fix_type="Quick"
if grep -q "server_tokens off;" "$NGINX_CONF" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -q "server_tokens on;" "$NGINX_CONF" > /dev/null 2>&1 || grep -q "server_names_hash_bucket_size" "$NGINX_CONF" > /dev/null 2>&1; then
    status="fail"
...
risk="Critical"
fix_type="Involved"
if grep -r "ssl_ciphers" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "HIGH:!aNULL:!MD5:!RC4" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="High"
fix_type="Involved"
if grep -r "ssl_protocols" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "TLSv1.2 TLSv1.3" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="High"
fix_type="Involved"
if grep -r "add_header Strict-Transport-Security" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "max-age=31536000" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Planned"
if grep -r "limit_except GET POST" "$NGINX_DIR" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Quick"
if grep -r "client_body_buffer_size" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "1m" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Quick"
if grep -r "client_max_body_size" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "10m" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -q "access_log" "$NGINX_CONF" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -q "error_log" "$NGINX_CONF" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Quick"
if [ -f "$NGINX_CONF" ] && [ "$(stat -c %a "$NGINX_CONF" 2>/dev/null)" -le 640 ] 2>/dev/null; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Quick"
if [ -d "$NGINX_DIR" ] && [ "$(stat -c %a "$NGINX_DIR" 2>/dev/null)" -le 755 ] 2>/dev/null; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Quick"
if [ -d "$NGINX_LOGS" ] && [ "$(stat -c %a "$NGINX_LOGS" 2>/dev/null)" -le 750 ] 2>/dev/null; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="High"
fix_type="Quick"
if grep -r "client_header_buffer_size" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "1k" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="High"
fix_type="Quick"
if grep -r "large_client_header_buffers" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "4 8k" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Planned"
if grep -r "limit_req_zone" "$NGINX_DIR" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Planned"
if grep -r "limit_conn_zone" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "zone=addr:10m" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Medium"
fix_type="Quick"
if grep -r "autoindex off;" "$NGINX_DIR" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -r "server_name" "$NGINX_DIR" > /dev/null 2>&1 | grep -v "^#" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Involved"
if grep -r "ssl_session_cache" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "shared:SSL:10m" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Involved"
if grep -r "ssl_session_timeout" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "10m" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -r "proxy_buffering on;" "$NGINX_DIR" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -r "proxy_buffer_size" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "8k" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Information"
fix_type="Quick"
if grep -r "gzip on;" "$NGINX_DIR" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -r "gzip_types" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "text/html text/plain" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -r "worker_processes" "$NGINX_CONF" > /dev/null 2>&1 | grep -E "auto|[0-9]+" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -r "worker_connections" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "1024" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -r "multi_accept off;" "$NGINX_DIR" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Low"
fix_type="Quick"
if grep -r "keepalive_timeout" "$NGINX_DIR" > /dev/null 2>&1 | grep -q "65" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
...
risk="Information"
fix_type="Quick"
if grep -r "sendfile on;" "$NGINX_DIR" > /dev/null 2>&1; then
    status="pass"
    remediation="N/A"
else
    status="fail"
